/* 合同审批页面 - 主逻辑 v2.1（含汇总+全状态编辑 + CDN容错） */
const API_BASE = "";
...
async function loadContracts() {
  try {
    ...
    let url = "/api/contracts/list?status=" + status;
...
loadContracts();
const evtSource = new EventSource("/api/contracts/events");
evtSource.onmessage = function(e) {
  if (e.data === "update") loadContracts();
};
