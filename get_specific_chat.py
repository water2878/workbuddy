...
def main():
    ...
    # 获取聊天信息
    print(f"\n[信息] 获取聊天信息...")
    chat_info = get_chat_info(token, TARGET_CHAT_ID)
    if chat_info.get("code") == 0:
        data = chat_info.get("data", {})
        chat_name = data.get("name") or "(未命名)"
        ...
        chat_name = "(未知)"
        print(f"  [警告] 无法获取聊天信息: {chat_info.get('msg')}")
    
    # 获取消息
    print(f"\n[获取] 聊天记录...")
    msgs = get_chat_messages(token, TARGET_CHAT_ID, start_ts, end_ts)
